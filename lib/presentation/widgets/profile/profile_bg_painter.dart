// lib/presentation/widgets/profile/profile_bg_painter.dart
//
// 🏛️ FONDO: Partículas isométricas flotantes (branding adaptativo)
// ────────────────────────────────────────────────────────────
...
class ProfileBgPainter extends CustomPainter {
  final double t;
  final Brightness brightness;

  ProfileBgPainter(this.t, this.brightness);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int row = -1; row <= 14; row++) {
      for (int col = -1; col <= 6; col++) {
        ...
        final alpha = (tileAlpha + pulse).clamp(0.0, isDark ? 0.10 : 0.06);
        final path =
            Path()
              ..moveTo(cx, cy - tH / 2)
              ..lineTo(cx + tW / 2, cy)
    ...
    for (final p in _particles) {
      ...
      final op = math.sin(phase * math.pi) * (isDark ? 0.28 : 0.18);
      if (op <= 0) continue;
      final px = p[0] * w;
  ...
  @override
  bool shouldRepaint(ProfileBgPainter o) =>
