// lib/presentation/widgets/auth/login_bg_painter.dart
//
// 🏛️ FONDO: Partículas isométricas + claraboya (branding adaptativo)
// ────────────────────────────────────────────────────────────
...
class LoginBgPainter extends CustomPainter {
  ...
  LoginBgPainter(this.t, this.brightness);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final tileAlpha = isDark ? 0.06 : 0.04;
    final tW = w * 0.18;
    final tH = tW * 0.5;
    for (int row = -1; row <= 9; row++) {
      for (int col = -1; col <= 6; col++) {
        ...
        final alpha = (tileAlpha + pulse).clamp(0.0, isDark ? 0.12 : 0.08);
        final path =
            Path()
              ..moveTo(cx, cy - tH / 2)
              ..lineTo(cx + tW / 2, cy)
        ...
        if (row % 3 == 0) {
          canvas.drawPath(
            path,
            Paint()
              ..color = diamondColor.withAlpha((alpha * 0.25 * 255).toInt()),
          );
        }
      }
    }
    ...
    for (final p in _particles) {
      ...
      final op = math.sin(phase * math.pi) * (isDark ? 0.35 : 0.25);
      if (op <= 0) continue;
      final px = p[0] * w;
  ...
  @override
  bool shouldRepaint(LoginBgPainter o) =>
