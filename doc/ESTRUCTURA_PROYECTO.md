...
## 🗃️ ESQUEMA DE BASE DE DATOS - TABLAS FALTANTES
...
**Relaciones:**
- **Producto ↔ Categoria** (N:1) - Muchos productos pertenecen a una categoría
- **Plazoleta ↔ Categoria** (1:1) - Una plazoleta puede tener una categoría principal

**Uso en el proyecto:**
- ✅ Entidad implementada: `lib/domain/entities/categoria.dart`
- ✅ Repositorio interface: `lib/domain/repositories/categoria_repository_interface.dart`
- ✅ Referenciada en entidad `Producto` como `categoria_id`
- ✅ Referenciada en entidad `Plazoleta` como `categoria_principal_id`

#### **2. `cache_invalidation_log`** - Registro de invalidación de caché
```sql
...
**Propósito:**
- Registrar cambios en tablas para sincronización de caché
- Soporte para estrategia offline-first
- Sincronización diferencial entre cliente y servidor

**Uso en el proyecto:**
- ⚠️ **NO implementado actualmente** en el código Flutter
- 🔄 **Potencial uso futuro:** Para sincronización avanzada de caché
- 📋 **Recomendación:** Considerar implementación en fase 2-3 del proyecto

### **🔗 RELACIONES CON OTRAS TABLAS:**

#### **Relaciones de `categoria`:**
1. **Producto → Categoria** (FK: `producto.categoria_id → categoria.id`)
   - ON DELETE CASCADE
   - ON UPDATE CASCADE

2. **Plazoleta → Categoria** (FK: `plazoleta.categoria_principal_id → categoria.id`)
   - ON DELETE SET NULL
   - ON UPDATE CASCADE

#### **Relaciones de `cache_invalidation_log`:**
- Tabla independiente sin relaciones directas
- Referencia lógica a otras tablas a través de campos `tabla` y `id_registro`
...
#### **Para `cache_invalidation_log`:**
```sql
CREATE INDEX idx_cache_invalidation_processed ON public.cache_invalidation_log USING btree (processed);
CREATE INDEX idx_cache_invalidation_timestamp ON public.cache_invalidation_log USING btree (timestamp DESC);
CREATE INDEX idx_cache_invalidation_tabla ON public.cache_invalidation_log USING btree (tabla);
```
...
#### **Para `cache_invalidation_log`:**
```sql
-- Trigger para limpiar logs antiguos procesados
...
CREATE TRIGGER trigger_cleanup_old_cache_logs
AFTER INSERT ON cache_invalidation_log
FOR EACH STATEMENT
EXECUTE FUNCTION cleanup_old_cache_logs();
```

### **📊 ANÁLISIS DE NECESIDAD EN EL PROYECTO:**

#### **Tabla `categoria`:**
- **✅ NECESARIA:** Ya está implementada y en uso
- **✅ Entidad Flutter:** Completamente implementada
- **✅ Relaciones:** Utilizada por Producto y Plazoleta
- **✅ Repositorio:** Interface definida, implementación pendiente

#### **Tabla `cache_invalidation_log`:**
- **⚠️ NO IMPLEMENTADA:** No hay referencias en código Flutter
- **🔮 USO FUTURO:** Útil para sincronización avanzada
- **📋 PRIORIDAD:** Baja - puede implementarse en fases posteriores
- **💡 ALTERNATIVA:** El `CacheService` actual usa TTL y limpieza periódica

### **🚀 RECOMENDACIONES DE IMPLEMENTACIÓN:**

1. **Prioridad Alta:**
   - Completar implementación de `CategoriaRepository`
   - Integrar categorías en pantallas de productos y tiendas
   - Implementar filtros por categoría

2. **Prioridad Media/Baja:**
   - Evaluar necesidad real de `cache_invalidation_log`
   - Considerar implementación si se requiere sincronización bidireccional
   - Implementar como parte de sistema de notificaciones push
